...
app.get("/user/:id", (req, res) => {
  const query =
    "select id, full_name, email, password, phone_no from user where id = ?";
...
app.get("/user/search/:email", (req, res) => {
  const query =
    "select id, full_name, email, password, phone_no from user where email = ?";
...
app.put("/user/update/:id", (req, res) => {
  const { full_name, email, password, phone_no } = req.body;
  const query =
    "update user set full_name = ?, email = ?, password = ?, phone_no = ? where id = ?";
  db.pool.execute(
    query,
    [full_name, email, password, phone_no, req.params.id],
    (err, result) => {
      if (err) {
        res.send(utils.createErrorResult(err));
      } else {
...
app.put("/blog/update/:id", (req, res) => {
  const { title, contents, user_id, category_id } = req.body;
  const query =
    "update blogs set title = ?, contents = ?, user_id = ?, category_id = ? where id = ?";
  db.pool.execute(
    query,
    [title, contents, user_id, category_id, req.params.id],
    (err, result) => {
      if (err) {
        res.send(utils.createErrorResult(err));
      } else {
...
app.get("/blogs/category/:id", (req, res) => {
  const query =
    "select id, title, contents, user_id, category_id from blogs where category_id = ?";
  db.pool.execute(query, [req.params.id], (err, blogs) => {
    if (err) {
      res.send(utils.createErrorResult(err));
    } else {
...
app.get("/blogs/details", (req, res) => {
  const query =
    "select b.id, b.title, b.contents, b.created_time, u.full_name, u.email, c.title as category_title, c.description as category_description from blogs b inner join user u on b.user_id = u.id inner join categories c on b.category_id = c.id";
  db.pool.execute(query, (err, blogs) => {
    if (err) {
      res.send(utils.createErrorResult(err));
    } else {
